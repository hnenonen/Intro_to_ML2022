...
```{python}
# picks every other column
seq = np.arange(0, npf_train_test.shape[1], 2)
npf_train_test = npf_train_test.iloc[:,seq]
```
...
```{python}
#X = npf_train_test.drop(["class2"], axis=1)
...
X_train, X_test, y_train, y_test = train_test_split(
    X, y, train_size=0.9, random_state=40, shuffle=True, stratify=npf_train_test["class2"]
)
...
```
```{python}
log_reg = LogisticRegression(penalty="l2", C=10_000, solver="lbfgs")
